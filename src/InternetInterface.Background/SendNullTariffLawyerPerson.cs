	...
	public class SendNullTariffLawyerPerson : Task
	{
		...
		protected override void Process()
		{
			var nullLeases = Session.Query<Lease>().Where(l =>
				...
				.ToList();
			var sndingLease =
				Session.Query<SendedLease>().Where(s => s.SendDate >= DateTime.Now.Date).Select(s => s.LeaseId).ToList();
			var smtp = new Mailer();
#if !DEBUG
			...
			foreach (var lease in nullLeases.Where(l => !sndingLease.Contains(l.Id))) {
				...
				text.AppendLine(
					string.Format("\"{0}\" пытается выйти в интернет, но из-за незаданной абонентской платы, получает серый Ip {1} .",
						lease.Endpoint.Client.Name, lease.Ip));
