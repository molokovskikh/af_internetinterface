		...
		public static string GetDescription(this object self, string fieldName = "")
		{
			...
			if (self as Enum != null) {
				...
				if (fieldInfoEnum != null) {
					var attributesEnum =
						(DescriptionAttribute[]) fieldInfoEnum.GetCustomAttributes(typeof (DescriptionAttribute), false);
			...
			if (self != null && fieldName == string.Empty) {
				var modelAttributes =
					(DescriptionAttribute[]) self.GetType().GetCustomAttributes(typeof (DescriptionAttribute), false);
