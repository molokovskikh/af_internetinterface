...
namespace InforoomControlPanel.Controllers
{
	...
	public class ServiceRequestController : ControlPanelController
	{
		...
		public ActionResult ServiceRequestList()
		{
			...
			criteria.CreateCriteria("Client", JoinType.InnerJoin);
			...
			if (!string.IsNullOrEmpty(pager.GetParam("ServiceMenFilter")) && pager.GetParam("ServiceMenFilter") != "0") {
				...
				criteria.CreateCriteria("ServicemenScheduleItem", "serviceManFor", JoinType.InnerJoin);
				//добавление условиий фильтрации
			...
			if ((!string.IsNullOrEmpty(pager.GetParam("RegionFilter"))) && pager.GetParam("RegionFilter") != "0")
			{
				...
				criteria.CreateCriteria("Client.PhysicalClient", "physicalClientFor", JoinType.InnerJoin);
				//объединение 'сервисной заявки' с 'адресом'
				criteria.CreateCriteria("Client.PhysicalClient.Address", JoinType.InnerJoin);
				//объединение 'сервисной заявки' с 'домом'
				criteria.CreateCriteria("Client.PhysicalClient.Address.House", "Region1", JoinType.InnerJoin);
				//объединение 'сервисной заявки' с 'улицей'
				criteria.CreateCriteria("Client.PhysicalClient.Address.House.Street", "Region2", JoinType.InnerJoin);
				//добавление условиий фильтрации
			...
			if (!string.IsNullOrEmpty(pager.GetParam("FreeFilter")))
				...
				criteria.Add(Restrictions.Eq("Free", pager.GetParam("FreeFilter") == "on"));
			// для фильтра: Поисковая фраза
			if (!string.IsNullOrEmpty(pager.GetParam("TextSearch")))
			{
				//объединение 'сервисной заявки' с 'работником'
				criteria.CreateCriteria("Employee", "employeeFor", JoinType.InnerJoin);
				//проверка, если 'сервисная заявка' еще не объединена с 'физ.клиентом'. Если нет, объединяем
				var newCriteria = criteria.GetCriteriaByAlias("physicalClientFor") ?? criteria.CreateCriteria("Client.PhysicalClient", "physicalClientFor", JoinType.InnerJoin);
			...
			if (pager.GetParam("DataFilter") != "BeginTime") {
				...
				var newCriteria = criteria.GetCriteriaByAlias("serviceManFor") ?? criteria.CreateCriteria("ServicemenScheduleItem", "serviceManFor", JoinType.InnerJoin);
				//добавление условиий фильтрации
