		...
		public void UpdateAddressByClient(int clientId, string path)
		{
			var propObj = DbSession.CreateSQLQuery(
				string.Format(" SELECT r.Region AS region, s.Name AS street, h.Number AS house,  a.Floor AS floor, " +
				              " a.Apartment AS apartment, a.Entrance AS entrance, p.Id  FROM regions AS r " +
				              " INNER JOIN  inforoom2_street AS s ON s.Region = r.Id" +
				              " INNER JOIN  inforoom2_house AS h ON h.Street = s.Id" +
				              " INNER JOIN  inforoom2_address AS a ON h.Id = a.house" +
				              " INNER JOIN  physicalclients AS p ON a.Id = p._Address" +
				              " INNER JOIN  clients AS c ON p.Id = c.PhysicalClient" +
