...
namespace InternetInterface.Models
{
	public class DlinkCommutateurInfo : CommutateurInfo, IPortInfo
	{
		...
		protected override List<Tuple<bool, string>> GetDataOfGeneralInformation(string data)
		{
			...
			for (int j = startIndex; j < stucturedData.Count; j++) concatenator.AddColumns(stucturedData[j]);
			for (int i = 0; i < columnCount; i++) {
		...
		private class ColumnConcatenator
		{
			...
			{
				string toReturn = "";
				foreach (var item in Lines) toReturn += item.GetTextOfColumn(columnNumber);
				return toReturn;
			}
		}
		...
		private class LineData
		{
			...
			public static void SetColumnsLength(string baseLine)
			{
				...
				for (int i = 0; i < splitedBaseLine.Length; i++) ColumnsLength[i] = splitedBaseLine[i].Length;
			}
			...
			{
				_iterator++;
				if (_iterator == TextArrays.Length) _iterator = 0;
